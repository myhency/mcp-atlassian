...
**Required Parameters:**
- **`issue_ids_or_keys`**: List of Jira issue IDs or keys, e.g. ['PROJ-123', 'PROJ-124']
...
**Required Parameters:**
- **`user_identifier`**: Identifier for the user (e.g., email address, username, or account ID)
...
**Required Parameters:**
- **`issue_key`**: Jira issue key (e.g., 'PROJ-123')
- **`time_spent`**: Time spent in Jira format (e.g., '1h 30m', '1d', '4h')
...
- **`url`**: The URL to link to (e.g., 'https://example.com/page')
- **`title`**: The title/name of the link (e.g., 'Documentation Page')
...
**Required Parameters:**
- **`page_id`**: Confluence page ID (numeric ID from URL)
...
**Required Parameters:**
- **`query`**: Search query - can be either simple text or CQL query string
...
**Required Parameters:**
- **`space_key`**: The key of the space to create the page in (e.g. 'DEV', 'TEAM', 'DOC')
